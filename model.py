...
class NUS_GAN(object):
    def __init__(self, sess, input_height=218, input_width=178, crop=True, batch_size=49, sample_num=49,
                 output_height=218, output_width=178, image_num = 32, y_dim=13, z_dim=100, gf_dim=64,
                 L1_lambda=100, L2_lambda1=50, L2_lambda2=50, L2_lambda3=50,
                 df_dim=64, gfc_dim=1024, dfc_dim=1024, dataset_name='KDEF', input_fname_pattern='*.JPG',
                 checkpoint_dir=None, train_num = 12, labels = np.ones(13, np.int),
        ...
        for epoch in range(config.epoch):
            ...
            for idx in range(0, batch_idxs):
                ...
                if self.y_dim:
                    ...
                    errD_loss = self.d_loss.eval({self.inputs:batch_images,
                                                       self.z:batch_z,
                                                       self.y:batch_y})                                                
                else:
                ...
                print("Epoch: [%2d/%2d] [%4d/%4d] time: %4.4f, d_loss: %.8f, g_loss: %.8f"
                      % (epoch, config.epoch, idx, batch_idxs,
                         time.time() - start_time, errD_loss, errG))
